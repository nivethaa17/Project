-- Climate Guard Database Schema

-- Create Database
CREATE DATABASE climate_guard;
USE climate_guard;
...
INSERT INTO departments (name, code, ministry, description) VALUES
...
INSERT INTO regions (region_name, state, district, region_code, latitude, longitude) VALUES
...
INSERT INTO weather_stations (station_name, station_code, latitude, longitude, elevation, district, state, region) VALUES
...
CREATE INDEX idx_weather_data_station_time ON weather_data(station_id, recorded_at DESC);
CREATE INDEX idx_alerts_severity_status ON alerts(severity, status);
CREATE INDEX idx_users_role_region ON users(role, region);
CREATE INDEX idx_disaster_reports_type_severity ON disaster_reports(report_type, severity);
